...
type HelmChartValues struct {
	...
	IncidentDetection imanifests.IncidentDetectionValues `json:"incidentDetection"`
}
...
func GetValuesFunc(ctx context.Context, k8s client.Client, logger logr.Logger) addonfactory.GetValuesFunc {
	return func(
		...
		if opts.Platform.IncidentDetection.Enabled {
			incDecOptions := ihandlers.BuildOptions(ctx, k8s, mcAddon, opts.Platform.IncidentDetection)
			userValues.IncidentDetection = *imanifests.BuildValues(incDecOptions)
		}
